...
const LoginForm = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response2 = await axios.post('http://127.0.0.1:8000/api/user', {}, { headers: { 'Authorization': 'Bearer ' + token } });

      const userRole = response2.data.usuario.role;

      // Redirigir según el rol del usuario
      if (userRole === 'admin') {
        console.log('Redirigiendo a /adminIndex');
        navigate('/adminIndex');
      } else {
        navigate('/MiEquipo');
      }
